...
import json
from gen.roll import lucky_item_pool_roll
    ...
    def handle_layer(self, ele):
        ...
        while ele["Type"] != "Category":
            ...
            if roll == "":
                ...
                print("Missed Subcategory Roll on Category: {}. Instance Chance: {} Cumalitive Chance: {}".format(ele["Name"], str(round(instance_probability, 5)*100)+"%",str(round(cumalitive_probability, 5)*100)+"%"))
                return ""
            instance_probability = roll[1]*float(ele["Chance"])
            cumalitive_probability = cumalitive_probability*instance_probability
            print('Rolled Subcategory: {} in Category: {}. Instance Chance: {} Cumalitive chance: {}'.format(roll[0]["Name"],ele["Name"],str(round(instance_probability, 5)*100)+"%",str(round(cumalitive_probability, 5)*100)+"%"))
            ele = roll[0]
        ...
        if ele["Type"] == "Category": 
            ...
            if roll == "":
                instance_probability = 1-float(ele["Chance"])
                cumalitive_probability = cumalitive_probability*instance_probability
                print("Missed Roll on Category: {}. Instance Chance: {} Cumalitive Chance: {}".format(ele["Name"], str(round(instance_probability, 5)*100)+"%",str(round(cumalitive_probability, 5)*100)+"%"))
                return ""
            asset_loc = assets_path + "/" + roll[0]
            instance_probability = roll[1]*float(ele["Chance"])
            cumalitive_probability = cumalitive_probability*instance_probability
            print('Rolled Asset: {} in Category: {}. Instance Chance: {} Cumalitive chance: {}'.format(roll[0],ele["Name"],str(round(instance_probability, 5)*100)+"%",str(round(cumalitive_probability, 5)*100)+"%"))
